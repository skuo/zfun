            ...
            if (f == null) {
                Callable<WeakReference<V>> eval = new Callable<WeakReference<V>>() {
                    public WeakReference<V> call() throws InterruptedException {
                        return c.computeWeakReference(arg);  // the main point
                    }
                };
                FutureTask<WeakReference<V>> ft = new FutureTask<WeakReference<V>>(eval);
